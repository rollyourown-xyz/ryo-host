...
if [ "$RESTORE_ALL" == "y" ]; then
  ...
  if [ "$RESTORE_ALL_SURE" == "yes" ]; then
    ...
    if [ "$RESTORE_ALL_STAMP_SURE" == "yes" ]; then
      ...
      # Stop and delete project containers if present
      echo ""
      echo "Stopping and deleting project containers on "$hostname" if present"
      for project in $PROJECTS; do
        /bin/bash "$SCRIPT_DIR"/../"$project"/scripts-project/stop-project-containers.sh -n "$hostname"
        /bin/bash "$SCRIPT_DIR"/../"$project"/scripts-project/delete-project-containers.sh -n "$hostname"
      done

      # Stop and delete module containers if present
      echo ""
      echo "Stopping and deleting module containers on "$hostname" if present"
      for module in $MODULES; do
        /bin/bash "$SCRIPT_DIR"/../"$module"/scripts-module/stop-module-containers.sh -n "$hostname"
        /bin/bash "$SCRIPT_DIR"/../"$module"/scripts-module/delete-module-containers.sh -n "$hostname"
      done
